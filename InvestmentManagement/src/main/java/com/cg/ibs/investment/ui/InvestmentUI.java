...
public class InvestmentUI {
						...
						while (choice != Menu.QUIT) {
							...
							if (ordinal >= 0 && ordinal < Menu.values().length) {
								...
								case BUY_GOLD:
									...
									Matcher matcher = pattern.matcher(Double.toString(goldUnits));
	...
	public void viewMyInvestments(String userId) {
		try {
			...
			List<MutualFund> InvstmntList = new ArrayList<MutualFund>(service.viewInvestments(userId).getFunds());
	...
	public void withdrawMFPlan(String userId) {
		...
		try {
			InvestmentList = new ArrayList<MutualFund>(service.viewInvestments(userId).getFunds());
